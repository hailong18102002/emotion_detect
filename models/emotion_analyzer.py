...
def analyze_face_emotions(image):
    ...
    try:
        ...
        font_size = FONTS["default"]["font_size"]
        ...
        # Tạo ảnh PIL từ ảnh OpenCV để vẽ text
        img_pil = Image.fromarray(cv2.cvtColor(img_to_draw, cv2.COLOR_BGR2RGB))
        ...
        for face_info in face_detections:
            ...
            # Vẽ bounding box (trên ảnh OpenCV)
            x, y, w, h = region['x'], region['y'], region['w'], region['h']
            cv2.rectangle(img_to_draw, (x, y), (x+w, y+h), (0, 255, 0), 2)
            ...
            # Đặt text phía trên bounding box sử dụng PIL
            text_y = y - 25 if y - 25 > 10 else y + h + 25
            draw.text((x, text_y), label_text, fill=(0, 255, 0), font=font)
        ...
        # Chuyển ảnh PIL trở lại định dạng OpenCV
        img_to_draw = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR)
        
    except ValueError as e:
        # Tạo ảnh PIL để hiển thị lỗi tiếng Việt
        img_pil = Image.fromarray(cv2.cvtColor(img_to_draw, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(img_pil)
        
        if "Face could not be detected" in str(e) or "No face detected" in str(e):
            draw.text((20, 40), "Không phát hiện khuôn mặt", fill=(0, 0, 255), font=font)
            print("Không phát hiện khuôn mặt")
        else:
            draw.text((20, 40), f"Lỗi: {str(e)}", fill=(0, 0, 255), font=font)
            print(f"Lỗi phân tích: {e}")
        ...
        draw.text((20, 40), "Lỗi không xác định", fill=(0, 0, 255), font=font)
        print(f"Lỗi không xác định: {e}")
